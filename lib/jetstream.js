...
async function requestAllDataFromCache() {
  ...
  // no payload.
  await nc.request("allData", Empty, {timeout: 1000})
  .then((reply) => {
    console.log(`requestAllDataFromCache reply received: ${sc.decode(reply.data)}`);
  })
  .catch((err)) => {
    console.log('requestAllDataFromCache no reply received')
  }
...
module.exports.requestAllDataFromCache = requestAllDataFromCache;